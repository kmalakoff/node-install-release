...
function worker(version, installPath, options, callback) {
  let platform = options.platform || 'local';
  if (platform === 'local') platform = process.platform;

  if (platform === 'win32') {
    assert.ok(existsSync(path.join(installPath, 'node.exe')), `node.exe ${installPath} ${fs.readdirSync(installPath)} ${fs.readdirSync(path.dirname(installPath))}`);
  ...
  const node = NODES[platform] || NODES.posix;
