...
module.exports = function installLib(version, dest, options, callback) {
  ...
  var bundledVersion = findVersion(versions, version).bundled;
  ...
  access(installPath, function (err) {
    ...
    mkdirp(installPath, function (err) {
      ...
      return download(DIST_URL + bundledVersion, installPath, { extract: true, strip: 1, progress: progress }, function (err) {
        console.log('');
        callback(err);
      });
    });
