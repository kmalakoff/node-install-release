...
function conditionalCopy(file, libPath, binPath, callback) {
  ...
  access(destPath, function (err) {
    ...
    access(sourcePath, function (err) {
      ...
      copyFile(sourcePath, destPath, callback);
    });
  });
}
...
module.exports = function installBin(version, dest, options, callback) {
  ...
  for (var index = 0; index < files.length; index++) {
    queue.defer(conditionalCopy.bind(null, files[index], libPath, binPath));
  }
